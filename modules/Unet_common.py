...
class DWT(nn.Module):
    def __init__(self):
        ...
        self.requires_grad = False # 设置 requires_grad 为 False，表示该模块在训练过程中不需要计算梯度

    def forward(self, x):
        return dwt_init(x) # 在前向传播过程中，对输入 x 执行离散小波变换并返回结果
...
class IWT(nn.Module):
    ...
    def forward(self, x):
        return iwt_init(x) # 在前向传播过程中，对输入 x 执行逆小波变换并返回结果
